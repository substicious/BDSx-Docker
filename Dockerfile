...
RUN cp $DEFAULT_CONFIG_PATH $DATA_PATH/configs
...
RUN easy-add --var version=1.3.35 --var app=bdsx --file {{.app}} --from https://github.com/itzg/{{.app}}/releases/download/{{.version}}/{{.app}}-{{.version}}-linux.tar.gz --to $SERVER_PATH
